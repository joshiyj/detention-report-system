...
import { generateReportBMED2, getDownloadUrl } from './services/api';
...
// ── AppBMED2 ─────────────────────────────────────────────────────────────────
export default function AppBMED2({ onBack }) {
  ...
  const [form, setForm] = useState({
    ...
    schoolName:  'School of Electrical & Electronics Engineering',
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      const data = await generateReportBMED2(fd);
      setResult(data);
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-amber-50 to-slate-100">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold tracking-wide px-2 py-0.5 bg-amber-50 text-amber-700 border border-amber-200 rounded-full uppercase shrink-0">
                  Biomed · 2nd Year (IV Sem)
                </span>
              </div>
      ...
      <main className="max-w-5xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-5 py-4 text-sm text-amber-800 space-y-1">
          <p className="font-semibold">📋 Biomedical Engineering — IV Semester Detention Report</p>
          <p className="text-xs text-amber-700">
            Expected columns: <span className="font-mono">Roll No. | Unique Id | Seat No | Student Name | OverAll Attendance | HAPE II | FAIML | SPA | BMECH | MIOT | PEIoT | INC | FED | IE | OE II | FAIML LAB | SPA LAB | DF3D | BCC | IE Lab</span>
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <section className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            ...
            <p className="text-xs text-slate-400 mb-4">
              Upload Biomedical Engineering 4th Semester Excel files (e.g. IV SEM BME.xls, IV SEM BME_1.xls). Multiple files will be parsed and merged.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  onChange={handleForm}
                  placeholder="e.g. UBEE26RS40004, UBEE26RS40005"
                  className="px-3.5 py-2.5 rounded-lg border border-slate-200 bg-white text-slate-800 text-sm
                             focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent
          ...
          >
            {loading ? (
              ...
              `⚡  Generate Biomedical 2nd Year Report${files.length > 1 ? ` (${files.length} sheets)` : ''}`
            )}
          </button>
        </form>
        ...
        {result && (
          <section className="bg-white rounded-2xl shadow-sm border border-green-200 p-6 space-y-6">
            <div className="flex items-center gap-2 text-green-600">
              <CheckIcon />
              <h2 className="text-base font-semibold">Biomedical 2nd Year Detention Report Generated Successfully</h2>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <StatCard label="Detained Subject Entries"    value={result.stats.detainedSubjects} color="purple" />
            </div>
