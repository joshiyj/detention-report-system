/**
 * wordGeneratorBMED3.js
 * Generates the 3rd Year (6th Semester) Biomedical Engineering Detention List .docx.
 * Matches the reference format of VI_Sem_BME_Final Detention_2025_26.docx
 *
 ...
 * Table III: Examination Seat No./ Student Unique No | Name of the students | Aggregate Attendance | Course Code (includes code and name) | Attendance (%)
 *
 * Sorting:
...
// Cols: Examination Seat No./ Student Unique No | Name of the students | Aggregate Attendance | Course Code | Attendance (%)
const T3_SEAT    = 2500;
const T3_NAME    = 2500;
...
// ── Main generator ────────────────────────────────────────────────────────────
async function generateWordBMED3(meta, { tableI, tableII, tableIII }, outputPath) {
  ...
  const doc = new Document({
    ...
    sections: [{
      ...
      children: [
        // ── University heading ─────────────────────────────────────────────────
        centerPara([bold('SHRI RAMDEOBABA COLLEGE OF ENGINEERING AND MANAGEMENT, NAGPUR', 24)], 40),
        centerPara([bold('Department of Electronics Engineering', 22)], 160),
        new Paragraph({
          alignment: AlignmentType.RIGHT,
          spacing: { after: 160 },
          children: [bold(`Date: ${date}`, 20)],
        }),
        new Paragraph({
          alignment: AlignmentType.LEFT,
          spacing: { after: 160 },
        ...
        new Paragraph({
          spacing: { after: 100 },
          children: [
            normal(`Table I is the list of students of programme ${programme} semester VI  having aggregate attendance less than 75%.`, 19),
          ],
        }),
        new Paragraph({
          spacing: { after: 160 },
        ...
        new Paragraph({
          spacing: { after: 160 },
          children: [
            normal(`However Table II, is the list of students of programme ${programme} semester VI having aggregate attendance between 60% and 75% and has applied for condonation of attendance. The application forms and medical certificates / other relevant documents have been attached herewith. After due consideration and as per the regulation R 17, it is recommended that these students may be permitted to appear in all courses in the `, 19),
            bold(examName, 19),
        ...
        new Paragraph({
          spacing: { after: 160 },
          children: [
            normal(`Table III is the list of students of programme ${programme} semester VI along with the course code and the attendance in the courses in which they are recommended to be detained in `, 19),
            bold(examName, 19),
        ...
        new Paragraph({
          spacing: { after: 40 },
          children: [
            bold('Dr.  Anju Gupta', 20),
            new TextRun({ text: '\t\t\t\t\t\t\t\t', size: 20 }),
            bold('Dr. Nitin Narkhede', 20),
          ],
        ...
        new Paragraph({
          spacing: { after: 0 },
          children: [
            bold('Academic Coordinator, EN', 20),
            new TextRun({ text: '\t\t\t\t\t\t\t\t', size: 20 }),
            bold('HoD , Department of Electronics Engineering', 20),
          ],
          tabStops: [{ type: 'right', position: 9026 }],
        }),
      ],
...
module.exports = { generateWordBMED3 };
